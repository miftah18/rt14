      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Pengguna Sistem</h1>
          </div>
          <button className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-medium transition">
            + Tambah Pengguna
          </button>
        </div>
      </div>
                ...
                {userList.length === 0 ? (
                  ...
                  userList.map(user => (
                    <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                      <td className="px-6 py-3 text-sm font-mono text-gray-900 dark:text-white">{user.username}</td>
                      <td className="px-6 py-3 text-sm text-gray-900 dark:text-white">{user.name}</td>
                      <td className="px-6 py-3 text-sm">
                        <span className="inline-block px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded text-xs font-medium capitalize">
                          {user.role}
                        </span>
                      </td>
                      <td className="px-6 py-3 text-sm text-gray-600 dark:text-gray-400">{user.organization?.name || '-'}</td>
                      <td className="px-6 py-3 text-sm">
